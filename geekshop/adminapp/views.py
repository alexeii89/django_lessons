...
def user_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if edit_form.is_valid():
            edit_form.save()
            return HttpResponseRedirect(reverse('adminapp:user_update',
                                                args=[edit_user.pk]))
    else:
...
def category_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if edit_form.is_valid():
            edit_form.save()
            return HttpResponseRedirect(reverse('adminapp:category_update',
                                                args=[update_category.pk]))
    else:
        edit_form = ProductCategoryEditForm(instance=update_category)
